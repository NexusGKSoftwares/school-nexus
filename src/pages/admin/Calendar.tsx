...
import { CalendarIcon, Plus, Edit, Clock, MapPin, Users, Loader2 } from "lucide-react"
...
export default function AdminCalendar() {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data: events, error: eventsError } = await calendarService.getCalendarEvents()
      ...
      if (events) {
        const transformedAcademicEvents: AcademicEvent[] = events
          ...
          .map((event) => ({
            ...
            title: event.title,
            date: event.event_date ? new Date(event.event_date).toISOString().split('T')[0] : "TBD",
  ...
  const thisMonthEvents = [...academicEvents, ...upcomingEvents].filter(event => {
    const eventDate = new Date(event.date)
    const now = new Date()
    return eventDate.getMonth() === now.getMonth() && eventDate.getFullYear() === now.getFullYear()
  }).length
  const upcomingCount = upcomingEvents.length
  const deadlinesCount = academicEvents.filter(event => event.type === "deadline").length
        ...
        <Card className="bg-white/80 backdrop-blur-sm border-blue-100 shadow-lg">
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-2xl font-bold text-gray-800">{thisMonthEvents}</div>
        ...
        <Card className="bg-white/80 backdrop-blur-sm border-blue-100 shadow-lg">
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-2xl font-bold text-gray-800">{deadlinesCount}</div>
