...
export default function AdminRegistrations() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (registrations) {
        const transformedRegistrations: Registration[] = registrations.map(
          (reg) => {
  ...
  const handleEditRegistration = (registration: Registration) => {
    ...
    if (originalData) {
      setSelectedRegistration({
        id: registration.id,
        student_id:
          students.find((s) => s.name === registration.studentName)?.id || "",
        course_id:
          courses.find((c) => c.name === registration.course)?.id || "",
  ...
  const pendingCount = registrationData.filter(
    (r) => r.status === "Pending",
  ).length;
  const approvedCount = registrationData.filter(
    (r) => r.status === "Approved",
  ).length;
  const rejectedCount = registrationData.filter(
    (r) => r.status === "Rejected",
  ).length;
                ...
                {filteredRegistrations.map((registration) => (
                  <TableRow key={registration.id}>
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={getPriorityColor(registration.priority)}
                      >
