...
import { Award, Search, Filter, Plus, Edit, Trash2, Eye, Loader2 } from "lucide-react"
...
import { scholarshipService, studentService, facultyService } from "@/lib/dataService"
...
export default function AdminScholarships() {
  ...
  const [scholarshipTypes, setScholarshipTypes] = useState<string[]>(["All Types"])
  const [scholarshipStatus, setScholarshipStatus] = useState<string[]>(["All Statuses"])
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch scholarships
      const { data: scholarships, error: scholarshipsError } = await scholarshipService.getScholarships()
      ...
      if (scholarships) {
        const transformedScholarships: Scholarship[] = scholarships.map((scholarship) => {
          const student = students?.find(s => s.id === scholarship.student_id)
          ...
          return {
            ...
            date: scholarship.created_at ? new Date(scholarship.created_at).toISOString().split('T')[0] : "N/A",
        ...
        const uniqueTypes = Array.from(new Set(transformedScholarships.map(s => s.name)))
        const uniqueStatuses = Array.from(new Set(transformedScholarships.map(s => s.status)))
  ...
  const handleSaveScholarship = async (data: any) => {
    try {
      ...
      if (selectedScholarship) {
        // Update
        const { error } = await scholarshipService.updateScholarship(selectedScholarship.id, data)
        if (error) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setIsSubmitting(true)
      const { error } = await scholarshipService.deleteScholarship(selectedScholarship.id)
      if (error) {
        toast({ title: "Error", description: error.message, variant: "destructive" })
        return
      }
    ...
    <div className="flex flex-1 flex-col gap-6 p-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <Button onClick={handleCreateScholarship} className="bg-gradient-to-r from-purple-500 to-violet-600 hover:from-purple-600 hover:to-violet-700 text-white">
                    ...
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <Avatar className="h-8 w-8">
                          <AvatarImage src={scholarship.avatar} alt={scholarship.studentName} />
                          <AvatarFallback>{scholarship.studentName.split(" ").map(n => n[0]).join("")}</AvatarFallback>
                        </Avatar>
                        <div>
                          <div className="font-medium text-gray-900">{scholarship.studentName}</div>
                          <div className="text-sm text-gray-600">{scholarship.email}</div>
                        </div>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuSeparator />
                          <DropdownMenuItem className="text-red-600" onClick={() => handleDeleteScholarship(scholarship)}>
      ...
      <ScholarshipModal
        ...
        students={scholarshipData.map(s => ({ id: s.studentId, name: s.studentName, email: s.email }))}
