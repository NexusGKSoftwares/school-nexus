  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: quizzesData, error: quizzesError } =
        await quizService.getQuizzesByLecturer(user.id);
      ...
      const { data: coursesData } = await courseService.getCoursesByLecturer(
        user.id,
      );
      ...
      if (quizzesData) {
        ...
        const mockAttempts: RecentAttempt[] = quizzesData
          .slice(0, 3)
          .map((quiz, index) => ({
    ...
    quizzes.filter((q) => q.status !== "draft").length > 0
      ? Math.round(
          quizzes
            ...
            .reduce((sum, quiz) => sum + (quiz.average_score || 0), 0) /
