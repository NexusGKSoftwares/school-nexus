  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (enrollments) {
        ...
        const lecturerIds =
          courses?.map((c) => c.lecturer_id).filter(Boolean) || [];
        const { data: lecturers } =
          await lecturerService.getLecturersByIds(lecturerIds);

        if (courses) {
          const transformedSchedule: ScheduleItem[] = courses.map(
            (course, index) => {
          ...
          const mockUpcomingClasses: UpcomingClass[] = transformedSchedule
            .slice(0, 3)
            .map((item, index) => ({
    ...
    <div className="flex flex-1 flex-col gap-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Manage your class timetable and upcoming sessions
          </p>
        </div>
                  ...
                  {scheduleData.map((class_, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell>
                        <Badge
                          variant={
                            class_.type === "Lab" ? "default" : "secondary"
                          }
                        >
                          {class_.type}
                        </Badge>
      ...
      <Card className="bg-white/80 backdrop-blur-sm border-blue-100 shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Monthly view of your academic schedule
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="text-center py-8">
